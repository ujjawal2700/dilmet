...
export const LocationPromptModal = ({
  ...
  const handleLocationChange = useCallback(
    (value: string, details?: LocationDetails) => {
      setLocation(value);
      if (details) {
  ...
  const handleUseMyLocation = async () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        if (err.code === 1)
          setError("Location access denied. Please enable in settings.");
        else if (err.code === 2)
          setError("Unable to determine location. Please enter manually.");
      ...
      <div className="bg-white dark:bg-[#342d18] rounded-2xl shadow-2xl p-8 max-w-md w-full mx-4">
        ...
        <div className="mb-6">
          ...
          <GoogleMapsAutocomplete
            ...
            className={`w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 dark:bg-[#2f151e] dark:text-white ${
