  ...
  return (
    <>
      ...
      <nav
        ...
        className="lg:hidden fixed bottom-0 inset-x-0 w-full z-50 bg-white/80 backdrop-blur-2xl backdrop-saturate-200 border-t border-white/80 shadow-[0_-8px_32px_rgba(0,0,0,0.06),0_-1px_0_rgba(255,255,255,0.9)_inset] transition-all duration-300 select-none"
      >
        ...
        <div className="relative max-w-md md:max-w-2xl mx-auto px-2 pt-2 pb-3">
          ...
          {activeIndex >= 0 && (
            ...
            >
              <div className="w-[64px] h-[50px] rounded-2xl bg-gradient-to-b from-pink-500/18 to-rose-500/10 backdrop-blur-md border border-pink-200/60 shadow-[inset_0_1px_1px_rgba(255,255,255,0.8),0_4px_14px_rgba(236,72,153,0.18)]" />
            </div>
          )}
                  ...
                  <div className="relative flex items-center justify-center">
                    <MaterialSymbol
                      ...
                      className={`transition-all duration-300 ${
                        isActive
                          ? 'text-pink-600 drop-shadow-[0_2px_8px_rgba(236,72,153,0.35)] scale-110'
                          : 'text-muted-light group-hover:text-slate-600 group-hover:scale-105'
                      }`}
                    />

                    {item.hasBadge && (
                      <span className="absolute -top-1 -right-1.5 h-2.5 w-2.5 rounded-full bg-pink-500 border-2 border-white shadow-sm animate-pulse" />
                    )}
                  </div>
                  ...
                  <span
                    className={`text-[10px] tracking-tight mt-0.5 transition-all duration-300 leading-none ${
                      isActive
                        ...
                        : 'font-semibold text-muted-light group-hover:text-slate-600'
                    }`}
                  >
                    {item.label}
                  </span>
      ...
      <div className="hidden lg:flex fixed left-0 top-0 h-screen w-60 z-50 bg-white/95 backdrop-blur-xl border-r border-slate-100 shadow-nav flex-col px-4 py-8">
            ...
            return (
              ...
              >
                <MaterialSymbol
                  ...
                  filled={isActive}
                  size={22}
                  className={isActive ? 'text-pink-600' : 'text-muted-light'}
                />
                ...
                {item.hasBadge && (
                  <span className="absolute top-3 right-4 h-2 w-2 rounded-full bg-pink-500 border border-white animate-pulse" />
                )}
              </button>
            );
          })}
