/**
 * Content Moderation Utility - Frontend
 * @purpose: Validate chat messages before sending to prevent sharing phone numbers (max 4 numbers allowed at a time)
 ...
 */
export const containsAbusiveContent = (text: string): boolean => {
    if (!text) return false;
...
export interface ModerationResult {
    isValid: boolean;
    reason?: 'phone_number' | 'abusive_language';
    message?: string;
}

/**
 * Validates message content against phone numbers (max 4 digits allowed) and profanity
 */
export const validateMessageContent = (content: string): ModerationResult => {
