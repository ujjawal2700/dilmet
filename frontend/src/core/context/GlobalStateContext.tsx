...
interface GlobalState {
    ...
    completedTask: import('../../shared/components/TaskCompletedModal').CompletedTaskInfo | null;
...
export const GlobalStateProvider = ({ children }: GlobalStateProviderProps) => {
    ...
    const [completedTask, setCompletedTask] = useState<import('../../shared/components/TaskCompletedModal').CompletedTaskInfo | null>(null);
    ...
    const handleTaskCompleted = useCallback((data: import('../../shared/components/TaskCompletedModal').CompletedTaskInfo) => {
    ...
    useEffect(() => {
        ...
        const timeout = setTimeout(() => {
            import('../services/task.service').then(({ default: taskService }) => {
                taskService.checkin().catch((e) => console.error('Daily task check-in failed:', e));
            });
        }, 2200);
        return () => clearTimeout(timeout);
    }, [user?.id, user?.role]);
